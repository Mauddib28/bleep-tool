...
def run_audio_recon(
    ...
    # Endpoint-contention summary — one line so recon reports flag the
    # condition even when audio playback isn't attempted.  Full detail is
    # available via ``audiocfg --endpoints``.  Failure is non-fatal.
    try:
        from bleep.core.preflight import check_endpoint_contention
        from bleep.bt_ref.constants import A2DP_SINK_UUID

        contention = check_endpoint_contention(
            A2DP_SINK_UUID, device_mac=mac_filter, deep_probe=False,
        )
        competitor_backends = sorted({
            o.backend for o in contention.competitors if o.backend != "bleep"
        })
        result["endpoint_contention"] = {
            "severity": contention.severity,
            "competitors": competitor_backends,
        }
        summary = (
            f"severity={contention.severity}"
            + (f" competitors={','.join(competitor_backends)}"
               if competitor_backends else "")
        )
        if contention.severity in ("warn", "block"):
            print_and_log(
                f"[!] Endpoint contention: {summary} "
                f"(in Debug Mode: 'audiocfg --endpoints')",
                LOG__USER,
            )
        else:
            print_and_log(
        ...
        # Pre-flight is advisory; keep the recon run going on any failure.
        print_and_log(
            f"[debug] endpoint-contention summary skipped: {exc}",
            LOG__DEBUG,
        )

    def _norm(m: str) -> str:
        return (m or "").replace(":", "").replace("-", "").upper()
    ...
    # If no data collected at all, note the gap with distinct reasons
    if not result["cards"] and not result["bluealsa_pcms"]:
        if backend == "none":
            ...
            print_and_log(
                f"[!] No audio device matching {mac_filter} found in backend",
                LOG__USER,
            )
            result["errors"].append("no_matching_device")
        else:
            print_and_log("[!] No Bluetooth audio devices found", LOG__USER)
            result["errors"].append("device_not_available")
...
def _recon_pulseaudio(
    ...
    for card in cards:
        ...
        original_profile = helper.get_active_profile_for_card(card_index)
        ...
        if profiles and card_index:
            non_off = [p for p in profiles if p.lower() != "off"]
            restore_to = original_profile or (non_off[0] if non_off else None)
            if restore_to:
                helper.set_card_profile(card_index, restore_to)
