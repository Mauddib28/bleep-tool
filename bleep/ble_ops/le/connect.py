...
def connect_and_enumerate__bluetooth__low_energy(
    ...
    *,
    ...
    skip_pair_fallback: bool
        If True, do not attempt automatic pairing when connection fails with
        NotAuthorized/Permission errors.  Instead re-raise so the caller can
        handle auth requirements explicitly.  Default False preserves the
        existing auto-pair behavior.
    ...
    except (_errors.NotAuthorizedError, _errors.PermissionError) as auth_exc:
        if skip_pair_fallback:
            print_and_log("[*] Connection requires pairing – skip_pair_fallback is set, re-raising", LOG__GENERAL)
            raise
