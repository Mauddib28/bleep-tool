...
> analysis here was **correct**: the `bus.get_object('org.bluez', '/test/agent')`
> introspection test is bogus (wrong service name + a process cannot introspect
> its own object), so its "empty XML" result never proved the `org.bluez.Agent1`
> methods were unregistered. Agent registration and dispatch work end-to-end
> (running-MainLoop fix, v2.6.2). Canonical:
...
> (`method=RequestConfirmation`, `result=success`).
