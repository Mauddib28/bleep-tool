...
> **✅ SUPERSEDED — ARCHIVED (2026-09-13).** Historical design sketch; the shipped
> implementation differs from this plan (e.g. `bleep/ble_ops/classic/pan.py`;
> `find_network_devices()` returns a `list`). See the current
> [Network capability summary](../network_capability_summary.md) and
> [PAN connection analysis](../pan_connection_analysis.md).

> **⚠️ Phase-numbering note (2026-07-12):** This document's phase scheme
> (Phase 1 = D-Bus wrapper; Phases 2–5 = enumeration / device-class helpers) is
> **distinct** from the authoritative
> [`todo_tracker.md`](../todo_tracker.md) → *"Bluetooth Networking (BlueZ PAN / BNEP)
> — Full D-Bus Incorporation Plan"* (Phase 1 = preflight **done**, Phase 2 =
> connect-path correctness **done**, Phase 3 = enumeration). The "Phases 2–5"
> here correspond to that tracker's **Phase 3**; treat the tracker as the source
> of truth for status.
>
> **✅ Status update (2026-07-12): tracker Phase 3 is COMPLETE, but the shipped
> API differs from the sketches below.** This document is a historical design
> exploration; the *implemented* design is:
> * Enumeration helpers live in **`bleep/ble_ops/classic/pan.py`** (not a new
>   `classic_network.py`): `find_network_servers()`, `find_network_devices()`,
>   `network_roles_from_uuids()`. They **return lists of dicts** and accept a
>   pre-fetched `GetManagedObjects` (single fetch, never raise) — not the
>   `Dict[path, ...]` / `Network` class shown here.
> * Device-class helpers on **both** `device_classic.py` and `device_le.py`:
>   `is_network_device()`, `has_network_uuids()`, `get_network_roles()`,
>   `get_network_status()`, `has_network_interface()`.
> * `get_device_info()` gains one additive **`network`** key
>   (`{roles, has_interface, status}` or `None`) — not the `network_capabilities`
>   sub-dict sketched below.
> * CLI: top-level `network-enum` (`--adapter/--json/--all-devices`).
> Refer to `bl_classic_mode.md` §2.11.1 and `changelog.md` for the authoritative
> shipped behaviour.
...
> `bleep/docs/bl_classic_mode.md` §2.11 for the actual implementation.
> **Update (2026-07-12):** Phases 2–5 below have since **shipped** (tracker Phase 3),
> via a different API than sketched here — see the ✅ status banner at the top of
> this file for the authoritative shipped surface. The sketches below are retained
> only as the original design record.

### Phase 2: Device Capability Detection — Superseded (shipped as tracker Phase 3)
...
### Phase 3: Network Enumeration Functions — Superseded (shipped as tracker Phase 3)
...
### Phase 4: CLI Integration — Superseded (shipped as `network-enum`)
...
### Phase 5: Update Device Info Methods — Superseded (shipped as the additive `network` key)
