...
> analysis here was **correct**: the `AccessDenied` on introspection is expected —
> a process **cannot introspect its own** bus object — so the "empty XML" test
> never proved the `org.bluez.Agent1` methods were unregistered. Agent
> registration and dispatch work end-to-end (running-MainLoop fix, v2.6.2).
> Canonical: [`agent_pairing_flow_analysis.md`](../agent_pairing_flow_analysis.md)
> (CONFIRMED WORKING). **Live re-confirmed (2026-07-23):** a fresh pairing run
