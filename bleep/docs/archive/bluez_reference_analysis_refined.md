...
> **not** treat the body below as current. Its premise — that BLEEP's
> `org.bluez.Agent1` methods "aren't being registered on D-Bus" — was a
> **misdiagnosis** (the empty / `AccessDenied` introspection XML came from
> introspecting the agent's **own** bus object and the wrong service name, not
> from unregistered methods). The one genuinely correct finding here — that the
> GLib **MainLoop must be running** for D-Bus to dispatch method calls — was the
> real fix (shipped v2.6.2). Agent registration and `Agent1` dispatch **work
> end-to-end** — canonical:
> [`agent_pairing_flow_analysis.md`](../agent_pairing_flow_analysis.md) (CONFIRMED
> WORKING). **Live re-confirmation (2026-07-23):** a fresh pairing run registered
> all 9 `Agent1` methods and paired a device end-to-end; the observation DB holds
> a `pairing_events` row (`method=RequestConfirmation`, `result=success`).
