...
> **✅ RESOLVED — ARCHIVED (2026-07-23).** Historical investigation record, and the
> one that identified the **real fix**: the GLib **MainLoop must be running** for
> D-Bus to dispatch `org.bluez.Agent1` method calls into Python (shipped v2.6.2 —
> `pair_device()` now spins a temporary MainLoop when no background loop exists).
> This superseded the earlier (incorrect) "methods not registered / empty XML"
> theory. Canonical:
> [`agent_pairing_flow_analysis.md`](../agent_pairing_flow_analysis.md) (CONFIRMED
> WORKING). **Live re-confirmed (2026-07-23):** a fresh pairing run logged
> `No background MainLoop – running temporary MainLoop for D-Bus dispatch`,
> registered all 9 `Agent1` methods, paired a device, and recorded a
> `pairing_events` row (`method=RequestConfirmation`, `result=success`).
