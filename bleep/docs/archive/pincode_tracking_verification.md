...
> **⚠️ ARCHIVED — SUPERSEDED (2026-09-13).** Historical verification guide. One
> prerequisite is **outdated**: agent registration does **not** enable unified
> D-Bus monitoring during pairing (that message filter blocks `Agent1` dispatch —
> see [Unified D-Bus Event Aggregator](../unified_dbus_event_aggregator.md) and
> `bleep/dbuslayer/agent.py::register`). Current PIN/passkey behaviour is covered
> by [Pairing Agent](../pairing_agent.md).
