...
> **✅ RESOLVED — ARCHIVED (2026-09-13).** Historical one-off analysis. Its
> conclusion is correct — the `AccessDenied` on **self-introspection** of the
> agent object path is expected and harmless — but it is superseded by the
> canonical [Agent Pairing Flow Analysis](../agent_pairing_flow_analysis.md).
