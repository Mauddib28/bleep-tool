...
> **✅ RESOLVED — ARCHIVED (2026-07-23).** Historical investigation record. The
> caution here was **correct**: the "empty introspection XML" never proved the
> methods were unregistered — it was an artifact of introspecting the agent's
> **own** bus object (`AccessDenied`) and of using the wrong service name. The
> open questions this doc raised are now **answered**: agent registration and
> `org.bluez.Agent1` dispatch work end-to-end (the running-MainLoop requirement
> was the real fix, v2.6.2). Canonical:
> [`agent_pairing_flow_analysis.md`](../agent_pairing_flow_analysis.md) (CONFIRMED
> WORKING). **Live re-confirmed (2026-07-23):** a fresh pairing run registered all
> 9 `Agent1` methods, paired a device, and recorded a `pairing_events` row
> (`method=RequestConfirmation`, `result=success`). Retained for context.
