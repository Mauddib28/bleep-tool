...
> **⚠️ ARCHIVED — SUPERSEDED (2026-07-23).** Historical investigation record; do
> **not** treat the body below as current. Its central claim — that BLEEP's
> `org.bluez.Agent1` methods "are not registered on D-Bus" (empty introspection
> XML) — was a **misdiagnosis**. The empty / `AccessDenied` XML came from a
> process introspecting **its own** bus object (a D-Bus security restriction) and
> from querying the wrong service name (`org.bluez` instead of BLEEP's unique bus
> name); it never proved the methods were unregistered. Agent registration and
> `Agent1` dispatch **work end-to-end** — canonical reference:
> [`agent_pairing_flow_analysis.md`](../agent_pairing_flow_analysis.md) (CONFIRMED
> WORKING, v2.6.2). **Live re-confirmation (2026-07-23):** a fresh pairing run
> registered all 9 `Agent1` methods and paired a device end-to-end; the
> observation DB holds a `pairing_events` row (`method=RequestConfirmation`,
> `pin=0000`, `result=success`) proving BlueZ invoked the agent callback.
