...
> **not** treat the body below as current. Its "Key Finding" — that BLEEP's
> `org.bluez.Agent1` methods "are not being registered on D-Bus" — was a
> **misdiagnosis**. The empty / `AccessDenied` introspection XML was an artifact
> of introspecting the agent's **own** bus object (a D-Bus security restriction)
> and of querying the wrong service name (`org.bluez` rather than BLEEP's unique
> bus name); it never proved the methods were unregistered. Agent registration
> and `Agent1` dispatch **work end-to-end** — canonical:
