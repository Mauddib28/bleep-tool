...
def _cmd_csdp(args: List[str]) -> None:
    """Perform SDP discovery on a Classic device (connectionless).
    
    Usage:
        csdp <MAC>                    # Regular SDP discovery
        csdp <MAC> --connectionless   # Verify reachability via l2ping first
        csdp <MAC> --connectionless --l2ping-count 5 --l2ping-timeout 20  # Custom l2ping params
    """
    global _current_device, _current_mapping, _current_mode, _current_path
    ...
    parser.add_argument("--connectionless", action="store_true", help="Verify device reachability via l2ping before SDP query")
    parser.add_argument("--l2ping-count", type=int, default=3, help="Number of l2ping echo requests (default: 3)")
    parser.add_argument("--l2ping-timeout", type=int, default=13, help="Seconds to wait for l2ping (default: 13)")
    ...
    try:
        ...
        if opts.connectionless:
            ...
            print_and_log(f"[*] Checking reachability via l2ping (count={opts.l2ping_count}, timeout={opts.l2ping_timeout}s)...", LOG__DEBUG)
            
            try:
                records = discover_services_sdp_connectionless(
                    mac,
                    timeout=30,
                    l2ping_count=opts.l2ping_count,
                    l2ping_timeout=opts.l2ping_timeout,
                )
        ...
        # Build service map from records
        ...
        if svc_map:
            ...
            # Optionally update global state if no device connected
            # This allows csdp to populate service map for later use
            ...
            elif _current_device.mac_address.upper() == mac:
                # Update service map if this is the connected device
...
def _cmd_pbap(args: List[str]) -> None:
    """Dump phonebook via PBAP from connected Classic device.
    
    Usage:
        pbap                                    # Dump PB repository to default location
        pbap --repos PB,ICH                     # Dump multiple repositories
        pbap --format vcard30 --auto-auth       # Use vCard 3.0 with auto-auth
        pbap --out /tmp/phone.vcf --watchdog 15 # Custom output with extended timeout
    """
    global _current_device, _current_mapping, _current_mode, _DB_AVAILABLE, _DB_SAVE_ENABLED, _obs
    
    # Validation: Check if Classic device is connected
    if _current_mode != "classic" or not _current_device:
        print("[-] No Classic device connected. Use 'cconnect <mac>' first")
        return
    
    # Parse arguments using argparse (following pattern from _cmd_enumb)
    parser = argparse.ArgumentParser(prog="pbap", description="Dump phonebook via PBAP")
    parser.add_argument("--repos", help="Comma-separated repository list (PB,ICH,OCH,MCH,CCH,SPD,FAV) or ALL", default="PB")
    parser.add_argument("--format", choices=["vcard21", "vcard30"], default="vcard21", help="vCard format")
    parser.add_argument("--auto-auth", action="store_true", help="Register temporary OBEX agent that auto-accepts authentication")
    parser.add_argument("--watchdog", type=int, default=8, help="Watchdog seconds before aborting stalled transfer (0 to disable)")
    parser.add_argument("--out", help="Output file path (for single repository only; multi-repo uses /tmp/<mac>_<repo>.vcf)")
    ...
    # Check PBAP service availability
    pbap_channel = None
    if _current_mapping:
        # Look for PBAP service in service map
        for key, ch in _current_mapping.items():
            low = key.lower()
            if (
    ...
    # If not found in service map, try SDP discovery
    if not pbap_channel:
        try:
    ...
    try:
        ...
        repos_tuple = DEFAULT_PBAP_REPOS if repos_arg == "ALL" else tuple(r.strip().upper() for r in repos_arg.split(",") if r.strip())
    except ImportError as exc:
    ...
    # Execute PBAP dump
    try:
        print_and_log(f"[*] Starting PBAP dump for {mac} (repos: {', '.join(repos_tuple)}, format: {opts.format})...", LOG__GENERAL)
        
        result = pbap_dump_async(
            mac,
            repos=repos_tuple,
            vcard_format=opts.format,
            auto_auth=opts.auto_auth,
        ...
        if result.get("success"):
            ...
            for repo, lines in data.items():
                ...
                try:
                    ...
                    if _DB_AVAILABLE and _DB_SAVE_ENABLED and _obs:
                        try:
                            import hashlib
                            with open(output_path, "rb") as fh:
                                vcf_bytes = fh.read()
                            vcf_hash = hashlib.sha1(vcf_bytes).hexdigest()
                            _obs.upsert_pbap_metadata(mac, repo, entry_count, vcf_hash)
                            print_and_log(f"[*] PBAP metadata saved to database", LOG__DEBUG)
